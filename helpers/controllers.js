...
const getDefaultBalances = (coins=[]) => {
    ...
    const defaultBalances = coins.map(coin => {
        return { coinId: coin.coinId, coinName: coin.coinName, coinTicker: coin.coinTicker, coinLogo: coin.coinLogo, coinStatus: coin.coinStatus, precision: getCoinPrecision(coin.coinName), balance: getPrecisionByCoin(0, coin.coinName) }
    })
